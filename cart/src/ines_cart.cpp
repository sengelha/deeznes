...
ines_cart::ines_cart(const char *filename) {
    ...
    cart_ = (uint8_t *)libc::mmap(
        NULL, round_up(si.st_size, libc::sysconf(_SC_PAGESIZE)), PROT_READ,
        MAP_FILE | MAP_SHARED, fd_.get(), 0);
    ...
    mapper_ = mapper_factory::create((cart_[7] & 0xF0) | (cart_[6] >> 4),
                                     prg_rom, chr_rom);
}

bool ines_cart::has_trainer() const { return (cart_[6] & 0x04); }

int ines_cart::mapper_number() const {
    return (cart_[7] & 0xF0) | (cart_[6] >> 4);
}

int ines_cart::prg_rom_size() const { return 16384 * cart_[4]; }

int ines_cart::chr_rom_size() const { return 8192 * cart_[5]; }
