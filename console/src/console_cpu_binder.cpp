...
uint8_t
console_cpu_binder::readu8(uint16_t address) const
{
	if (address < 0x0800)
	{
	...
	else if (address >= 0x8000 && address <= 0xFFFF && console_->cart_.has_value())
	{
		return console_->cart_->readu8(address);
	}
	else
	{
		std::cerr << "Warning: Unable to read from address $" << hex << setw(4) << setfill('0') << address << '\n';
		assert(false);
		return 0xFF;
	}
}
...
void console_cpu_binder::writeu8(uint16_t address, uint8_t val)
{
	...
	{
		std::cerr << "Warning: Unable to write to address $" << hex << setw(4) << setfill('0') << address << '\n';
		assert(false);
	}
}
