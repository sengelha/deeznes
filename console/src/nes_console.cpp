...
std::ostream &operator<<(std::ostream &os, const nes_console &console) {
  ...
  os << hex << uppercase << setw(4) << setfill('0')
     << (int)state.cpu_state.regs.PC << "  ";
  ...
  os << std::string(9 - 3 * instr.num_bytes, ' ')
     << (instr.non_standard ? '*' : ' ') << instr.opcode << " ";

  switch (instr.addrmode) {
  case m6502_addrmode::ABS: {
    uint16_t addr = (instr.bytes[2] << 8) | instr.bytes[1];
    os << "$" << hex << uppercase << setw(4) << setfill('0') << (int)addr
       << " ";

    static constexpr m6502_opcode no_curr_val_opcodes[] = {m6502_opcode::JMP,
                                                           m6502_opcode::JSR};
    if (find(begin(no_curr_val_opcodes), end(no_curr_val_opcodes),
             instr.opcode) == end(no_curr_val_opcodes)) {
      os << "= " << hex << uppercase << setw(2) << setfill('0')
         << (int)console.cpu_binder_.readu8(addr) << "                 ";
    } else {
      os << "                     ";
    }
    break;
  ...
  case m6502_addrmode::ABX: {
    ...
    os << "$" << hex << uppercase << setw(4) << setfill('0') << (int)addr
       << ",X @ " << hex << uppercase << setw(4) << setfill('0') << (int)addr2
  ...
  case m6502_addrmode::ABY: {
    ...
    uint8_t val = console.cpu_binder_.readu8(addr2);
    os << "$" << hex << uppercase << setw(4) << setfill('0') << (int)addr
       << ",Y @ " << hex << uppercase << setw(4) << setfill('0') << (int)addr2
       << " = " << hex << uppercase << setw(2) << setfill('0') << (int)val
       << "        ";
    break;
  }
  ...
  case m6502_addrmode::IND: {
    uint16_t addr = (instr.bytes[2] << 8) | instr.bytes[1];
    uint16_t val = (console.cpu_binder_.readu8(addr + 1) << 8) |
                   console.cpu_binder_.readu8(addr);
    os << "($" << hex << uppercase << setw(4) << setfill('0') << addr
  ...
  case m6502_addrmode::IXX: {
    ...
    uint16_t addr = (console.cpu_binder_.readu8((addend + 1) & 0xFF) << 8) |
                    console.cpu_binder_.readu8(addend);
    uint8_t val = console.cpu_binder_.readu8(addr);
    os << "($" << (int)instr.bytes[1] << ",X) @ " << (int)addend << " = " << hex
       << uppercase << setw(4) << setfill('0') << addr << " = " << hex
       << uppercase << setw(2) << setfill('0') << (int)val << "   ";
    break;
  }
  case m6502_addrmode::IXY: {
    uint16_t val1 =
        (console.cpu_binder_.readu8((instr.bytes[1] + 1) & 0xFF) << 8) |
        console.cpu_binder_.readu8(instr.bytes[1]);
    uint16_t val2 = val1 + state.cpu_state.regs.Y;
    uint8_t val3 = console.cpu_binder_.readu8(val2);
    os << "($" << (int)instr.bytes[1] << "),Y = " << hex << uppercase << setw(4)
       << setfill('0') << (int)val1 << " @ " << hex << uppercase << setw(4)
       << setfill('0') << (int)val2 << " = " << hex << uppercase << setw(2)
       << setfill('0') << (int)val3 << " ";
    break;
  }
  ...
  case m6502_addrmode::ZPG: {
    ...
    os << "$" << hex << uppercase << setw(2) << setfill('0')
       << (int)instr.bytes[1] << " = " << hex << uppercase << setw(2)
       << setfill('0') << (int)curr_val << "                   ";
    break;
  }
  case m6502_addrmode::ZPX: {
    ...
    uint8_t val = console.cpu_binder_.readu8(addr);
    os << "$" << hex << uppercase << setw(2) << setfill('0')
       << (int)instr.bytes[1] << ",X @ " << hex << uppercase << setw(2)
       << setfill('0') << (int)addr << " = " << hex << uppercase << setw(2)
       << setfill('0') << (int)val << "            ";
    break;
  }
  case m6502_addrmode::ZPY: {
    ...
    uint8_t val = console.cpu_binder_.readu8(addr);
    os << "$" << hex << uppercase << setw(2) << setfill('0')
       << (int)instr.bytes[1] << ",Y @ " << hex << uppercase << setw(2)
       << setfill('0') << (int)addr << " = " << hex << uppercase << setw(2)
       << setfill('0') << (int)val << "            ";
    break;
  }
  ...
  os << " "
     << "A:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.cpu_state.regs.A << " "
     << "X:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.cpu_state.regs.X << " "
     << "Y:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.cpu_state.regs.Y << " "
     << "P:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.cpu_state.regs.P << " "
     << "SP:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.cpu_state.regs.S << " "
     << "CYC:" << dec << setw(3) << setfill(' ') << (int)state.ppu_state.cycles
     << " "
     << "SL:" << dec << (int)state.ppu_state.scanline << " "
     << "C:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.ppu_state.regs.ppuctrl << " "
     << "M:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.ppu_state.regs.ppumask << " "
     << "S:" << hex << uppercase << setw(2) << setfill('0')
     << (int)state.ppu_state.regs.ppustatus;
  return os;
}
...
uint_fast32_t nes_console::run(uint_fast32_t maxcycles) {
...
nes_console_state nes_console::state() const {
