...
static ostream &print_state(ostream &os, nes_console &console) {
  ...
  case m6502_addrmode::IXX: {
    ...
    uint16_t addr =
        (binder.readu8((addend + 1) & 0xFF) << 8) | binder.readu8(addend);
  ...
  case m6502_addrmode::IXY: {
    uint16_t val1 = (binder.readu8((instr.bytes[1] + 1) & 0xFF) << 8) |
                    binder.readu8(instr.bytes[1]);
