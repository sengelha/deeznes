...
class m6502
{
    ...
    inline void writeu8abx(uint8_t val) { cb_->writeu8(readu16imm() + state_.regs.X, val); }
    inline void writeu8aby(uint8_t val) { cb_->writeu8(readu16imm() + state_.regs.Y, val); }
    ...
    inline void writeu8ixy(uint8_t val) { cb_->writeu8(readu16pgwrap(readu8imm()) + state_.regs.Y, val); }
...
public:
    ...
    m6502(const m6502_state &state, m6502_callbacks *cb) : state_(state), cb_(cb) {}
    m6502(m6502_state &&state, m6502_callbacks *cb) noexcept : state_(std::move(state)), cb_(cb) {}
